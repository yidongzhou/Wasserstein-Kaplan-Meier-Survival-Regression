...
for(j in 1:length(rhoVec)) {
  ...
  for(l in 1:length(ctVec)) {
    ...
    sewm[[j]][[l]] <- se[seq(1, 2 * Q, 2), ]# WKM
    secm[[j]][[l]] <- se[seq(2, 2 * Q, 2), ]# CPH
  }
}
save(sewm, secm, file = 'data/sem.RData')
...
for(j in 1:length(rhoVec)) {
  ...
  for(l in 1:length(ctVec)) {
    ...
    sewl[[j]][[l]] <- se[seq(1, 2 * Q, 2), ]# WKM
    secl[[j]][[l]] <- se[seq(2, 2 * Q, 2), ]# CPH
  }
}
save(sewl, secl, file = 'data/sel.RData')
stopCluster(cl)