source("code/wkmSim.R")

# Parallel computing
library(doSNOW)
cl <- makeCluster(50) 
registerDoSNOW(cl)
clusterEvalQ(cl, library(survival))
progress <- function(q) {
  if(q %% 10 == 0){
    cat(sprintf("%d runs are complete\n", q))
  }
}

Q <- 100
M <- 5000

d <- 50
k <- 2# shape parameter
beta <- rep(1, d) / 100
p <- rep(0.5, d)# success probability of each covariate
nVec <- c(100, 200, 500)# sample size
rhoVec <- c(0.05, 0.1, 0.5)# random variation
ctVec <- c(2, 1)# censoring rate (2: 20%; 1: 50%)

# generate random sample sizes N_i
set.seed(1)
N <- list()
for(i in 1:length(nVec)) {
  lambdan <- nVec[i]
  cn <- 0.5
  Ni <- NULL
  while (length(Ni) < nVec[i]) {
    Ni <- c(Ni, rpois(1, lambda = cn * lambdan))
    if (Ni[length(Ni)] < 1) {
      Ni <- Ni[-length(Ni)]
    }
  }
  N[[i]] <- Ni
}

#####################
### Setting I WKM ###
#####################
sewh1 <- list()
sech1 <- list()
set.seed(1)
for(j in 1:length(rhoVec)) {
  rho <- rhoVec[j]
  sewh1[[j]] <- list()
  sech1[[j]] <- list()
  for(l in 1:length(ctVec)) {
    ct <- ctVec[l]
    se <- foreach(n = nVec, .combine = 'cbind') %:%
      foreach(icount(Q), .combine = 'c', .options.snow = list(progress = progress)) %dopar% {
        idx <- which(nVec == n)
        x <- sapply(p, function(pi) rbinom(n, 1, pi))# n by p
        colnames(x) <- paste0('X', 1:d)
        yMean <- t(apply(x, 1, function(xi)
          qweibull(1:(M - 1) / M, shape = k, scale = sum(xi * beta) + 0.1)))# conditional mean
        dt <- list()
        y <- matrix(nrow = n, ncol = M)
        for (i in 1:n) {
          lambda <- rgamma(1, shape = (sum(x[i, ] * beta) + 0.1)^2 / rho,
                           scale = rho / (sum(x[i, ] * beta) + 0.1))
          u <- rweibull(N[[idx]][i], shape = k, scale = lambda)
          cs <- rweibull(N[[idx]][i], shape = k, scale = ct * lambda)
          while(all(u > cs)) {
            cs <- rweibull(N[[idx]][i], shape = k, scale = ct * lambda)
          }
          dt[[i]] <- data.frame(
            time = pmin(u, cs),
            death = as.integer(u <= cs),
            t(replicate(N[[idx]][i], x[i, ])))# X1, ..., Xd
          sc <- survfit(Surv(time, death) ~ 1, data = dt[[i]])
          obs <- cbind(sc$time, sc$surv)
          wt <- diff(c(0, 1 - obs[, 2]))
          sp <- c(obs[wt != 0, 1], max(dt[[i]]$time))# Efron
          wt <- wt[wt != 0]
          yi <- rep(sp, M * c(wt, 1 - sum(wt)))
          if (length(yi) < M) {
            yi <- c(yi, sample(sp, M - length(yi), prob = c(wt, 1 - sum(wt))))
          }
          y[i, ] <- sort(yi)
        }
        system.time(res <- wkm(y, x, x))
        
        dt <- do.call('rbind', dt)
        sc <-
          survfit(coxph(Surv(time, death) ~ ., data = dt), newdata = as.data.frame(x))
        y0 <- list()
        wt0 <- list()
        for (i in 1:nrow(x)) {
          obs <- cbind(sc$time, sc$surv[, i])
          wt <- diff(c(0, 1 - obs[, 2]))
          y0i <- obs[wt != 0, 1]
          wt <- wt[wt != 0]
          if (sum(wt) >= 1) {
            if(length(wt) > 1) {
              y0i <- y0i[-length(y0i)]
              wt <- wt[-length(wt)]
            } else {# if a point mass
              y0i <- rep(y0i, M - 1)
              wt <- 1:(M - 1) / M
            }
          }
          y0[[i]] <- y0i
          wt0[[i]] <- wt
        }
        
        c(sum((res$qf[, -M] - yMean) ^ 2) / (M - 1),# WKM
          sum(sapply(1:nrow(x), function(i) {
            weighted.mean((y0[[i]] - qweibull(
              cumsum(wt0[[i]]),
              shape = k,
              scale = sum(x[i, ] * beta) + 0.1
            )) ^ 2, w = wt0[[i]]) / sum(wt0[[i]])
          }))) / nrow(x)# CPH
      }
    sewh1[[j]][[l]] <- se[seq(1, 2 * Q, 2), ]# WKM
    sech1[[j]][[l]] <- se[seq(2, 2 * Q, 2), ]# CPH
  }
}
save(sewh1, sech1, file = 'data/seh1.RData')

######################
### Setting II CPH ###
######################
sewh2 <- list()
sech2 <- list()
set.seed(1)
for(j in 1:length(rhoVec)) {
  rho <- rhoVec[j]
  sewh2[[j]] <- list()
  sech2[[j]] <- list()
  for(l in 1:length(ctVec)) {
    ct <- ctVec[l]
    se <- foreach(n = nVec, .combine = 'cbind') %:%
      foreach(icount(Q), .combine = 'c', .options.snow = list(progress = progress)) %dopar% {
        idx <- which(nVec == n)
        x <- sapply(p, function(pi) rbinom(n, 1, pi))# n by d
        colnames(x) <- paste0('X', 1:d)
        yMean <- t(apply(x, 1, function(xi) 
          qweibull(1:(M - 1) / M, shape = k, scale = exp(-sum(xi * beta) / 2))))
        dt <- list()
        y <- matrix(nrow = n, ncol = M)
        for (i in 1:n) {
          lambda <- rgamma(1, shape = exp(-sum(x[i, ] * beta)) / rho, 
                           scale = rho / exp(-sum(x[i, ] * beta) / 2))
          u <- rweibull(N[[idx]][i], shape = k, scale = lambda)
          cs <- rweibull(N[[idx]][i], shape = k, scale = ct * lambda)
          while(all(u > cs)) {
            cs <- rweibull(N[[idx]][i], shape = k, scale = ct * lambda)
          }
          dt[[i]] <- data.frame(
            time = pmin(u, cs),
            death = as.integer(u <= cs),
            t(replicate(N[[idx]][i], x[i, ])))# X1, ..., Xd
          sc <- survfit(Surv(time, death) ~ 1, data = dt[[i]])
          obs <- cbind(sc$time, sc$surv)
          wt <- diff(c(0, 1 - obs[, 2]))
          sp <- c(obs[wt != 0, 1], max(dt[[i]]$time))# Efron
          wt <- wt[wt != 0]
          yi <- rep(sp, M * c(wt, 1 - sum(wt)))
          if (length(yi) < M) {
            yi <- c(yi, sample(sp, M - length(yi), prob = c(wt, 1 - sum(wt))))
          }
          y[i, ] <- sort(yi)
        }
        res <- wkm(y, x, x)
        
        dt <- do.call('rbind', dt)
        sc <-
          survfit(coxph(Surv(time, death) ~ ., data = dt), newdata = as.data.frame(x))
        y0 <- list()
        wt0 <- list()
        for (i in 1:nrow(x)) {
          obs <- cbind(sc$time, sc$surv[, i])
          wt <- diff(c(0, 1 - obs[, 2]))
          y0i <- obs[wt != 0, 1]
          wt <- wt[wt != 0]
          if (sum(wt) >= 1) {
            if(length(wt) > 1) {
              y0i <- y0i[-length(y0i)]
              wt <- wt[-length(wt)]
            } else {# if a point mass
              y0i <- rep(y0i, M - 1)
              wt <- 1:(M - 1) / M
            }
          }
          y0[[i]] <- y0i
          wt0[[i]] <- wt
        }
        
        c(sum((res$qf[, -M] - yMean) ^ 2) / (M - 1),# WKM
          sum(sapply(1:nrow(x), function(i) {
            weighted.mean((y0[[i]] - qweibull(
              cumsum(wt0[[i]]), shape = k, scale = exp(-sum(x[i,] * beta) / 2)
            )) ^ 2, w = wt0[[i]]) / sum(wt0[[i]])
          }))) / nrow(x)# CPH
      }
    sewh2[[j]][[l]] <- se[seq(1, 2 * Q, 2), ]# WKM
    sech2[[j]][[l]] <- se[seq(2, 2 * Q, 2), ]# CPH
  }
}
save(sewh2, sech2, file = 'data/seh2.RData')
stopCluster(cl)
