...
for(j in 1:length(rhoVec)) {
  ...
  yMean <- t(apply(xOut, 1, function(xOuti) 
    qweibull(1:(M - 1) / M, shape = k, scale = exp(-sum(xOuti * beta) / 4))))
  for(l in 1:length(ctVec)) {
    ...
    se <- foreach(n = nVec, .combine = 'cbind') %:%
      foreach(icount(Q), .combine = 'c', .options.snow = list(progress = progress)) %dopar% {
        ...
        for (i in 1:n) {
          lambda <- ifelse(rho > 0, rgamma(1, shape = exp(-sum(x[i, ] * beta) / 2) / rho, 
                                           scale = rho / exp(-sum(x[i, ] * beta) / 4)), 
                           exp(-sum(x[i, ] * beta) / 4))
        ...
        c(sum((res$qf[, -M] - yMean) ^ 2) / (M - 1),# WKM
          sum(sapply(1:nrow(xOut), function(i) {
            weighted.mean((y0[[i]] - qweibull(
              cumsum(wt0[[i]]), shape = k, scale = exp(-sum(xOut[i,] * beta) / 4)
            )) ^ 2, w = wt0[[i]]) / sum(wt0[[i]])
          }))) / nrow(xOut)# CPH
      }
    sewb[[j]][[l]] <- se[seq(1, 2 * Q, 2), ]# WKM
    secb[[j]][[l]] <- se[seq(2, 2 * Q, 2), ]# CPH
  }
}
save(sewb, secb, file = 'data/seb.RData')
stopCluster(cl)