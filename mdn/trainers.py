...
from mdn_models import SurvivalMDN_Dist
...
class MDNTrainer(object):
    ...
    def get_mixture_params(self, x):
        self.model.eval()
        with torch.no_grad():
            pred_params = self.model.model(x)
            dist = SurvivalMDN_Dist(pred_params)
            ws, mus, sigmas = dist.pred_params_to_parameters(pred_params)
